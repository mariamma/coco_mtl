...
def train(model, train_loader, optimizer, device, args):
    ...
    for images, targets in train_loader:
        ...
        det_loss, seg_loss, kp_loss, _ = model(images, targets)
        ...
        if 'keypoints' in args.tasks:
            ...
            if torch.isnan(kp_losses) == False:   
        ...
        loss = 0.0
        for task in tasks:
            if torch.isnan(task_loss_dict[task]) == False and task_loss_dict[task] !=None:
                loss += task_loss_dict[task]
        loss.backward()
    ...
    return det_loss_dict, seg_loss_dict, kp_loss_dict
...
def save_model(models, epoch, args, folder="saved_models/", name="best"):
    ...
    epoch = str(epoch)
    torch.save(state, f"{folder}{run_name}_epoch{epoch}_{name}_model.pkl")   
...
def main():
    ...
    if 'keypoints' in tasks:
        dataset = CocoKeypointsDataset(datasetDir='/data6/rajivporana_scratch/coco_2017', split='train2017' )
        val_dataset = CocoKeypointsDataset(datasetDir='/data6/rajivporana_scratch/coco_2017', split='val2017')
    else:
        dataset = CocoInstancesDataset(datasetDir='/data6/rajivporana_scratch/coco_2017', split='train2017')
        val_dataset = CocoInstancesDataset(datasetDir='/data6/rajivporana_scratch/coco_2017', split='val2017')
    ...
    for epoch in range(n_epochs):
        print(f"Epoch {epoch}")
        det_loss_dict, seg_loss_dict, kp_loss_dict = train(model, train_loader, optimizer, device, args)
        ...
        if not args.debug:
            wandb.log({'detection_loss':{k: v for k, v in det_loss_dict.items()}}, step=epoch)
            wandb.log({'segmentation_loss':{k: v for k, v in seg_loss_dict.items()}}, step=epoch)
            wandb.log({'keypoints_loss':{k: v for k, v in kp_loss_dict.items()}}, step=epoch)
        ...
        if not args.debug and map_val>max_map:
            max_map = map_val
...
"""
det_loss ([], {'loss_objectness': tensor(3.1161, device='cuda:2',
       grad_fn=<BinaryCrossEntropyWithLogitsBackward0>),
         'loss_rpn_box_reg': tensor(0.4997, device='cuda:2', grad_fn=<DivBackward0>),
           'loss_classifier': tensor(0.5246, device='cuda:2', grad_fn=<NllLossBackward0>),
             'loss_box_reg': tensor(0.0463, device='cuda:2', grad_fn=<DivBackward0>)})

seg_loss ([], {'loss_objectness': tensor(1.3073, device='cuda:2',
       grad_fn=<BinaryCrossEntropyWithLogitsBackward0>), 'loss_rpn_box_reg': tensor(0.5132, device='cuda:2', grad_fn=<DivBackward0>), 
       'loss_classifier': tensor(0.2672, device='cuda:2', grad_fn=<NllLossBackward0>), 'loss_box_reg': tensor(0.0281, device='cuda:2', grad_fn=<DivBackward0>),
         'loss_mask': tensor(1.2530, device='cuda:2',
       grad_fn=<BinaryCrossEntropyWithLogitsBackward0>)})
kp_loss None
